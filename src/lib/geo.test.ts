...
import { haversineKm, travelMin, RADIUS_KM, DEFAULT_LOCATION } from "@/lib/geo";
...
describe("RADIUS_KM", () => {
  it("is fixed per mode — walking stays around the point, never 5+ km", () => {
    expect(RADIUS_KM.walking).toBe(2.5);
    expect(RADIUS_KM.transit).toBe(12);
    expect(RADIUS_KM.car).toBe(24);
  });
});
